...
def setup_bolt(tmp_path, bolt_id="test-bolt", sealed=True):
    ...
    if sealed:
        ...
        identity_json = (
            '{"signer_id": "tester", "public_key": "key", "signature": "sig"}'
        )
        events.append(
            f'{{"action": "SEAL_INTENT", "identity": {identity_json}, "sequence": 1, "timestamp": "2023-01-01T00:00:00Z", "event_hash": "abc", "prev_hash": "0000", "payload": {{}} }}'
        )

    ledger_content = (
        f'{{"bolt_id": "{bolt_id}", "created_at": "2023-01-01T00:00:00Z", "head_hash": "abc", "events": ['
        + ",".join(events)
        + "]}"
    )
...
def test_prove_command_success(tmp_path):
    ...
    with patch("geas_ai.commands.prove.run_tests") as mock_run:
        ...
        with patch("geas_ai.commands.prove.ensure_geas_root", return_value=tmp_path):
            with patch(
                "geas_ai.commands.prove.get_active_bolt_name", return_value="test-bolt"
            ):
