...
from geas_ai.schemas.verification import ViolationCode
...
def create_signed_event(sequence, prev_hash, action, payload, signer_name, ctx):
    ...
    event_identity = EventIdentity(
        signer_id=signer_name, public_key=pub_key, signature=signature
    )
...
def test_chain_integrity_valid(test_ctx):
    ...
    e2 = create_signed_event(
        2,
        e1.event_hash,
        LedgerAction.SEAL_SPECS,
        {"file": "specs.md", "hash": "h2"},
        "human-dev",
        test_ctx,
    )
...
def test_workflow_compliance_valid(test_ctx, mock_workflow):
    ...
    e3 = create_signed_event(
        3,
        e2.event_hash,
        LedgerAction.SEAL_PLAN,
        {"file": "plan.md", "hash": "h3"},
        "agent-ai",
        test_ctx,
    )
    ...
    ledger = Ledger(
        ...
        created_at=datetime.now(timezone.utc),
        events=[e1, e2, e3],
        head_hash=e3.event_hash,
    )

    result = verification.validate_workflow_compliance(
        ledger, mock_workflow, test_ctx.store
    )
    assert result.valid
...
def test_chain_sequence_gap(test_ctx):
    """Test non-contiguous sequence numbers."""
    e1 = create_signed_event(
        1,
        None,
        LedgerAction.SEAL_REQ,
    ...
    result = verification.validate_chain_integrity(ledger)
    assert not result.valid
    assert any(v.code == ViolationCode.SEQUENCE_GAP for v in result.violations)


def test_signature_identity_not_found(test_ctx):
    """Test signer not present in identity store."""
    # We need to manually create an event with unknown signer because create_signed_event relies on ctx.store
    # Hack: create valid event, then patch signer_id
    e1 = create_signed_event(
        1,
        None,
        LedgerAction.SEAL_REQ,
    ...
    e1.identity.signer_id = "unknown-signer"

    # Recalculate hash because identity changed (identity is part of event structure usually? No, EventIdentity is separate)
    # LedgerEvent has `identity` field. `calculate_event_hash` includes `identity`.
    # So we must re-calculate hash.
    event_dict = e1.model_dump(mode="json")
    del event_dict["event_hash"]
    e1.event_hash = calculate_event_hash(event_dict)
    ...
    result = verification.validate_signatures(ledger, test_ctx.store)
    assert not result.valid
    assert any(v.code == ViolationCode.IDENTITY_NOT_FOUND for v in result.violations)
