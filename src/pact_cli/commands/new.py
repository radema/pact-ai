...
def new(name: str):
    ...
    try:
        ...
        with open(ctx_path, "w") as f:
            f.write(content.CONTEXT_TEMPLATE.format(bolt_name=name, timestamp=timestamp))

        # 6. Success Output
        console.print(Panel(f"[bold green]Bolt Started![/bold green]\n\nWorkspace: [blue]{bolt_dir}[/blue]\nContext: [yellow]Updated[/yellow]", title=f"Bolt: {name}"))
